...
class BasicPlace(nn.Module):
    ...
    def plot(self, params, placedb, iteration, pos, path=None, hpwl=None):
        """
        @brief plot layout
        @param params parameters
        @param placedb placement database
        ...
        if not params.optimization:
            figname = os.path.join(path, f'{params.benchmark}_{hpwl/1e7:.4f}.png')
        else:
            figname = os.path.join(path, f'{params.call_id}.png')
